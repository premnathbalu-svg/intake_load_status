...
completed_df = completed_df.dropna(subset=["processed_row_count", "start_timestamp", "updated_ts"])
...
completed_df["institution"] = completed_df["source_table"].str.replace("_intake_loadstatus", "", regex=False)

# Optional filters
st.sidebar.header("Filters")
if "table_source" in completed_df.columns:
    table_sources = sorted(completed_df["table_source"].dropna().unique())
    selected_sources = st.sidebar.multiselect("Table Source", table_sources, default=table_sources)
...
if "table_type" in completed_df.columns:
    ...
    selected_types = st.sidebar.multiselect("Table Type", table_types, default=table_types)
...
with viz1:
    ...
    fig_duration.update_layout(yaxis_title="Duration in Minutes")
...
with viz2:
    ...
    fig_records.update_layout(yaxis_title="Processed Row Count")
...
display_cols = [
    "institution", "source_table", "table_type", "table_source", "status",
    "processed_row_count", "start_timestamp", "updated_ts", "duration_minutes", "duration_hours"
]
display_cols = [c for c in display_cols if c in completed_df.columns]

show_df = completed_df[display_cols].sort_values("duration_minutes", ascending=False).copy()
